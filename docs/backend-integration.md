...
### ⚠️ 시각에 `Z` 가 없습니다 — 서버가 고칩니다

지금 서버는 `"2026-09-16T07:44:02.912496"` 처럼 타임존 표시 없이 내려줍니다.
JS 의 `new Date()` 는 이런 문자열을 **로컬 시각으로 해석**하므로, 한국에서는
모든 시각이 **9시간 어긋납니다** (채팅 시각, "○분 전", 알림 시각 전부).

**백엔드에서 `...Z` 를 붙이도록 고치겠습니다.** 프론트는 지금처럼 `new Date(isoDate)` 를
그대로 쓰시면 됩니다. 고치기 전에 먼저 붙여보신다면 이 어긋남이 프론트 버그가 아닙니다.

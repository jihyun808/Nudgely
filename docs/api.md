...
| `type`              | 언제                                       | 이동 대상    |
| ------------------- | ------------------------------------------ | ------------ |
| `nudge`             | AI가 **독촉 목적으로** 먼저 말을 걸었을 때 | 해당 채팅방  |
| `todoAdded`         | 투두가 추가됐을 때                         | 해당 채팅방  |
| `todoDone`          | 투두가 완료됐을 때                         | 해당 채팅방  |
| `todoIncomplete`    | 밤 11시까지 미완료 투두가 남았을 때        | 기록 탭      |
| `plannerIncomplete` | 밤 11시까지 플래너가 비었을 때             | 기록 탭      |

- `title`은 AI 이름이나 목표 이름, `body`가 본문이다.
- **`linkTo`는 서버가 내려줘야 한다.** 알림을 누르면 이 경로로 이동하는 동작은 이미 구현돼 있다.
  ⚠️ 다만 경로 문자열을 서버가 박으면 프론트 라우팅이 바뀔 때마다 서버도 배포해야 하므로,
  **`targetType`(`goal` | `record`) + `targetId`로 내려주고 프론트가 경로를 조립하는 쪽을 권한다.** 합의 필요.
...
3. **푸시(FCM/APNs)** — 앱을 닫은 동안에도 받으려면 필수. **Capacitor 전환 이후**에 가능하고 기기 토큰 등록 API(`POST /devices`)가 함께 필요하다.

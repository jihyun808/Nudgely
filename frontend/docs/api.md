...
- **`progress`가 없으면** 프론트는 진행률 막대 대신 `2026. 6. 4.부터 진행 중이에요`를 보여준다. 진도를 셀 기준이 없는 목표(예: 매일 습관)를 위한 처리다.
...
프론트 타입: `TodoMark` (`src/types/record.ts`)

- **완료한 항목 하나가 꽃잎 하나**다. 그 항목이 속한 목표 id를 항목 수만큼 넣어 준다(같은 목표에서 3개를 완료했으면 같은 id가 세 번).
- 최대 4장까지 그린다. 그 이상 완료해도 꽉 찬 꽃으로 표시된다.
- **색은 목표별로 고정**된다(프론트가 목표 id로 정함). 목표가 하나여도 많이 할수록 꽃이 커지고, 여러 목표를 챙기면 색이 섞여 진해진다.
- 완료한 항목이 없는 날은 빈 배열이거나 아예 내려주지 않으면 된다.
- 캘린더에서 달을 넘길 때마다 그 달로 다시 조회한다.

> 나중에 목표 설정에서 사용자가 색을 직접 고르게 하려면 `Goal`에 `color` 필드를 추가하고 프론트의 자동 색 대신 그 값을 쓰면 된다.
...
**프론트는 SDK 없이 브라우저 리다이렉트로 인가 코드를 받는다**(`src/lib/oauth.ts`).

1. 버튼 클릭 → 제공자 인가 페이지로 이동 (`client_id`, `redirect_uri`, `state` 포함)
2. 제공자가 `/auth/callback/:provider?code=...&state=...`로 돌려보냄
3. 프론트가 `state`를 검증한 뒤 `code`를 `POST /auth/social`로 넘김
4. **서버가 제공자에게 코드를 토큰으로 교환하고**, 계정을 만들거나 찾아 우리 토큰을 발급

필요한 준비:

| 항목                                                            | 담당   |
| --------------------------------------------------------------- | ------ |
| 카카오·구글 개발자 콘솔에 앱 등록, 클라이언트 ID 발급           | 팀     |
| 리다이렉트 URI 등록 (`{도메인}/auth/callback/kakao`, `/google`) | 팀     |
| `.env`에 `VITE_KAKAO_CLIENT_ID`, `VITE_GOOGLE_CLIENT_ID` 설정   | 프론트 |
| 코드 → 토큰 교환, 계정 생성·연결                                | 백엔드 |

- **클라이언트 ID가 없으면** 버튼을 눌러도 "소셜 로그인은 준비 중이에요" 토스트만 뜨고 이동하지 않는다.
- `state`는 세션 스토리지에 저장했다가 돌아올 때 대조한다(CSRF 방지).
